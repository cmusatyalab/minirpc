...
{
	return minirpc_send_request(conn, nr_list_parcels, in, out);
}
...
{
	return minirpc_send_request_async(conn, nr_list_parcels, in,
				callback, private);
}

int send_list_parcels_async_reply(struct minirpc_message *request,
			int status, ListParcelsReply *out)
{
	return minirpc_send_reply(request, status, out);
}
...
{
	return minirpc_conn_set_operations(conn, protocol, ops);
}
